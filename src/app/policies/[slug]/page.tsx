...
export default async function PolicyPage({
  ...
  const policy = await prisma.policyPage.findFirst({
  ...
  return (
    <article className="max-w-2xl mx-auto px-4 py-10">
      <p className="text-sm mb-2">
        <Link href="/policies" className="underline underline-offset-4 opacity-80">
          All policies
        </Link>
      </p>
      <h1 className="font-display text-4xl">{policy.title}</h1>
      <YarnDivider />
      <div className="leading-relaxed opacity-95">{renderContent(policy.content)}</div>
    </article>
  );
}
